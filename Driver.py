...
class Driver:
    ...
    def load_standing_locations(self)->Coordinates:
        '''
        ...
        for line in lines:
            ...
            if len(coords) == 2:
                try:
                    ...
                    self.standing_locations.append((x, y,self.pointcloudholder.find_altitude((x,y),4)))
                except ValueError:
                    print(f"Invalid coordinates: {coords}")
    ...
    def generate_points(self,starting_point:Coordinate=None,points=None)->Coordinates:
        ...
        while len(cities) < self.number_points:
            ...
            if self.standing_locations:
                if self.standing_locations[self.current_standing_id]:
                    if not self.pointcloudholder.two_points_visible(self.standing_locations[self.current_standing_id],(x,y)):
                        continue
            cities.append((x, y))
        self.cities = cities
        return cities
    ...
    def solve_tsp(self)->None:
        ...
        '''
        cutoff = int(self.config.config['speed_related']['brute_foce_cutoff'])
