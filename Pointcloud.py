...
class PointCloud:
    ...
    def distance_to_nearest_point(self,
        ...
        return int(np.linalg.norm(np.array(point)-np.array(nearest_point)))
    ...
    def two_points_visible(self,
        ...
        length = int(np.linalg.norm(np.array(point1)-np.array(point2))/10)
        ...
        for i,_ in enumerate(line_between_two_points):
            ...
            if self.distance_to_nearest_point(line_between_two_points[i]) < minimum_dvlos:
                print(self.distance_to_nearest_point(line_between_two_points[i]))
                visible = False
                break
        return visible
    def interpolate_route(self,
                          route:XYCoordinates,
                          number_of_interpolations:int=10)-> XYZCoordinates:
        interpolated_route = []
        ...
        for i in range(len(route)-1):
                interpolated_x = np.linspace(route[i][0],route[i+1][0],num=10)
                interpolated_y = np.linspace(route[i][1],route[i+1][1],num=10)
    ...
    def show_point_cloud(self,
        ...
        if best_path_coords:
            ...
            for i in range(len(interpolated_route)-1):
                self.add_line_to_pointcloud(interpolated_route[i],interpolated_route[i+1],[1,0,0])
